...
Short-circuit syntax is a way to make your code shorter and easier to read. It uses logical operators like && (AND) and || (OR) to decide what should happen — without needing an if statement.

```js
const user = { name: "Emma" };

user && console.log(user.name); // Only run this if 'user' exists
const nickname = user.nickname || "Guest"; // Set a default value if 'nickname' is missing
console.log(nickname); // Output: "Guest"
